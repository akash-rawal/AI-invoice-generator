...
const AllInvoice = () => {
  ...
  const handleDelete = async (id) =>{
    if(window.confirm('Are you sure you want to delete this invoive?')){
      try {
        await axiosInstance.delete(API_PATHS.INVOICE.DELETE_INVOICE(id));
        setInvoices(invoices.filter(invoice=> invoice._id !== id));
      } catch (err) {
        setError('Failed to delere invoice.')
        console.error(err);        
      }
    }
  }
  ...
  const handleStatusChange = async (invoice) =>{
    ...
    try {
      ...
      const response = await axiosInstance.put(API_PATHS.INVOICE.UPDATE_INVOICE(invoice._id),updateInvoice);
...
const filterdInvoices = useMemo(() => {
  return invoices
    .filter(
      (invoice) =>
        statusFilter === "All" ||
        invoice.status.trim().toLowerCase() === statusFilter.trim().toLowerCase()
    )
    .filter(
      (invoice) =>
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="p-4 rounded-lg bg-red-50 border border-e-red-200">
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
        <div className="p-4 sm:p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-grow">
              ...
              <input type="text" placeholder='Search by Invoice # or Client...'
            ...
            <div className="flex-shrink-0">
              <select  className="w-full sm:w-auto h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm text-slate-900 placeholder-slate-400 resize-vertical focus:outline-none focus:ring-2 focus:ring-blue-500 " 
              value={statusFilter} 
        ...
        {filterdInvoices.length === 0 ?(
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            {invoices.length === 0 && (
              <Button onClick={()=> navigate('/invoices/new')} icon={Plus}>Create Firt Invoice</Button>)
            }
          </div>
            ...
            <div className="w-[90vw] md:w-auto overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 tracking-wider uppercase">Invoice #</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 tracking-wider uppercase">Client</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 tracking-wider uppercase">Amount</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 tracking-wider uppercase">Due Date</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 tracking-wider uppercase">Status</th>
                      <th className="px-6 py-3 text-right text-xs font-medium text-slate-500 tracking-wider uppercase">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white  divide-y divide-slate-200">
                    {filterdInvoices.map(invoice=>(
                      <tr key={invoice._id} className='hover:bg-slate-50'> 
                        ...
                        <td onClick={()=>navigate(`/invoices/${invoice._id}`)} className="px-6 py-4 whitespace-nowrap text-sm  text-slate-900 cursor-pointer">{invoice.billTo.clientName}</td>
                        <td onClick={()=>navigate(`/invoices/${invoice._id}`)} className="px-6 py-4 whitespace-nowrap text-sm  text-slate-900 cursor-pointer">{invoice.total.toFixed(2)}</td>
                        <td onClick={()=>navigate(`/invoices/${invoice._id}`)} className="px-6 py-4 whitespace-nowrap text-sm  text-slate-900 cursor-pointer">{moment(invoice.dueDate).format('MMM D, YYYY')}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span
                            ...
                            : invoice.status.trim().toLowerCase() === "pending"
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                             <div className="flex items-center justify-end gap-2" onClick={(e)=> e.stopPropagation()}>
                              <Button
                              size='small' variant='secondary' onClick={()=>handleStatusChange(invoice)}
                              ...
                              {invoice.status !== 'Paid' && (
                                <Button size='small' variant='ghost' onClick={()=>handleOpenReminderModal(invoice._id)} title="Generate Remider"><Mail className='w-4 h-4 text-blue-500'/></Button>
                              )}
