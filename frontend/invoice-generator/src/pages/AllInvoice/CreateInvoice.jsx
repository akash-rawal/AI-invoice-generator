...
const CreateInvoice = ({existingInvoice,onSave}) => {
  ...
  const [formData , setFormData] = useState(
    ...
    const handleSubmit = async(e)=>{
      ...
      const finalFormData = {...formData,items: itemWithTotal,subtotal,taxTotal,total};
      if(onSave){
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {formData.items.map((item,index)=>(
                <tr key={index} className="hover:bg-slate-50">
                  ...
                  <td className="px-2 sm:px-6 py-4">
                    <input type="number" name='unitPrice' value={item.unitPrice} onChange={(e)=> handleInputChange(e,null,index)} className="w-full h-10 px-3 py-2 border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder='0.00' />
                  </td>
                  <td className="px-2 sm:px-6 py-4">
                    <input type="number" name='taxPercent' value={item.taxPercent} onChange={(e)=> handleInputChange(e,null,index)} className="w-full h-10 px-3 py-2 border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder='0' />
                  </td>
        ...
        <div className="p-4 sm:p-6 border-t border-slate-200 ">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 space-y-4 ">
              ...
              <SelectField label="Payment Terms" name="paymentTerms" value={formData.paymentTerms} onChange={handleInputChange} options={["Net 15","Net 30","Net 60","Due on recepit"]}/>
            </div>
